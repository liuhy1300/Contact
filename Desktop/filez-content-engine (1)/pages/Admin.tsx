...
import { Plus, Trash2, Save, RefreshCw, X, Edit2, Users } from 'lucide-react';
import UserManagement from '../components/UserManagement';

const CATEGORIES: { key: CategoryKey | 'users'; label: string }[] = [
  ...
  { key: 'users', label: '用户管理 (Admin Only)' },
  // Expanded Options
  { key: 'cmsOptions', label: 'CMS 兼容 (CMS)' },
  { key: 'multimodalOptions', label: '多模态 (Multimodal)' },
  { key: 'headlineStrategies', label: '标题策略 (Headlines)' },
  { key: 'wordCounts', label: '篇幅 (Word Count)' },
  { key: 'languages', label: '语言 (Language)' },
  { key: 'geoStructures', label: 'GEO 结构 (GEO)' },
  { key: 'imageStyles', label: '图片风格 (Img Style)' },
  { key: 'imageRatios', label: '图片比例 (Img Ratio)' },
  { key: 'marketingHooks', label: '心理钩子 (Hooks)' },
];

interface AdminSidebarProps {
  activeCategory: string;
  setActiveCategory: (key: any) => void;
  resetData: () => void;
}

const AdminSidebar: React.FC<AdminSidebarProps> = ({ activeCategory, setActiveCategory, resetData }) => (
  <>
      ...
      {CATEGORIES.map(cat => (
        <li key={cat.key}>
          <button
            ...
            className={`w-full text-left px-4 py-3 text-xs font-medium border-l-4 transition-all ${activeCategory === cat.key
              ? 'border-blue-500 bg-blue-50 text-blue-700'
              : 'border-transparent text-slate-600 hover:bg-slate-50'
              }`}
          >
            {cat.key === 'users' && <Users className="w-3 h-3 inline mr-1 mb-0.5" />}
            {cat.label}
          </button>
        </li>
      ))}
...
const Admin: React.FC = () => {
  ...
  const [activeCategory, setActiveCategory] = useState<CategoryKey | 'users'>('roles');
  ...
  // Helper to safely get current items
  const getCurrentItems = (): BaseOption[] => {
    if (activeCategory === 'users') return [];
    return (data[activeCategory as CategoryKey] || []) as BaseOption[];
  };
  ...
  const handleSaveEdit = () => {
    if (editingId && tempItem && activeCategory !== 'users') {
      updateItem(activeCategory as CategoryKey, editingId, tempItem);
      setEditingId(null);
      setTempItem(null);
    }
  };
  ...
  const handleDelete = (id: string) => {
    if (activeCategory !== 'users' && window.confirm('确认删除此选项吗？')) {
      deleteItem(activeCategory as CategoryKey, id);
    }
  };
  ...
  const handleAddItem = () => {
    ...
    if (activeCategory !== 'users') {
      addItem(activeCategory as CategoryKey, newItem);
  ...
  return (
    <div className="flex h-full bg-slate-50">
      ...
      <div className="w-48 md:w-64 bg-white border-r border-slate-200 overflow-y-auto">
        <AdminSidebar activeCategory={activeCategory} setActiveCategory={setActiveCategory} resetData={resetData} />
      </div>
